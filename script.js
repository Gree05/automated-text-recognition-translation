...
async function handleFileUpload() {
    ...
    if (extractedText) {
        ...
        if (detectedLang !== targetLang) {
            if (['hi', 'es'].includes(detectedLang)) { // Only translate if it's Hindi or Spanish
                ...
                // For languages other than Hindi and Spanish, display original text
                document.getElementById("output").innerText = `Original Text:\n${extractedText}`;
            }
        } else {
            // If the detected language is already the target language, no need to translate
